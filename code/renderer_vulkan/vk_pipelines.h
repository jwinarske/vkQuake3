...
struct GlobalPipelineManager {
	...
	// Standard pipelines.
	//
	pipelineDef skybox_pipeline;

	// dim 0: 0 - front side, 1 - back size
	// dim 1: 0 - normal view, 1 - mirror view
	pipelineDef shadow_volume_pipelines[2][2];
	pipelineDef shadow_finish_pipeline;

	// dim 0 is based on fogPass_t: 0 - corresponds to FP_EQUAL, 1 - corresponds to FP_LE.
	// dim 1 is directly a cullType_t enum value.
	// dim 2 is a polygon offset value (0 - off, 1 - on).
	pipelineDef fog_pipelines[2][3][2];

	// dim 0 is based on dlight additive flag: 0 - not additive, 1 - additive
	// dim 1 is directly a cullType_t enum value.
	// dim 2 is a polygon offset value (0 - off, 1 - on).
	pipelineDef dlight_pipelines[2][3][2];

	// debug visualization pipelines
	pipelineDef tris_debug_pipeline;
	pipelineDef tris_mirror_debug_pipeline;
	pipelineDef normals_debug_pipeline;
	pipelineDef surface_debug_pipeline_solid;
	pipelineDef surface_debug_pipeline_outline;
	pipelineDef images_debug_pipeline;
};
